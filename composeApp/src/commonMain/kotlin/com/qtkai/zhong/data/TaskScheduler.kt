...
import com.qtkai.zhong.sendHeartbeatNotification
import com.qtkai.zhong.sms.SmsPoller
import com.qtkai.zhong.ui.markdown.parseMarkdown
import com.qtkai.zhong.ui.markdown.toSpeakableText
...
class TaskScheduler(
     ...
     */
    fun start() {
        if (!enabled || taskStore == null || appSettings == null) {
            println("TaskScheduler: start skipped (enabled=$enabled taskStore=${taskStore != null} appSettings=${appSettings != null})")
            return
        }
        if (activeJob?.isActive == true) return
        println("TaskScheduler: starting scheduler loop")
        activeJob = schedulerScope.launch {
            while (isActive) {
                delay(POLL_INTERVAL_MS.milliseconds)
                if (!appSettings.isSchedulingEnabled()) {
                    println("TaskScheduler: scheduling disabled, skip")
                    continue
                }

                val dueTasks = taskStore.getDueTasks()
                println("TaskScheduler: poll - dueTasks=${dueTasks.size} heartbeatDue=${heartbeatManager?.isHeartbeatDue()} isLoading=${isLoadingCheck()}")
                ...
                if (!isLoadingCheck() && heartbeatManager?.isHeartbeatDue() == true) {
                    println("TaskScheduler: heartbeat due, running")
                    runHeartbeat()
                }
