## v3.2.8 — 2026-09-13 (Kai2 9000 正式版)
### Features（UI 体验大优化）
- 聊天消息去边框：AI 回复/用户消息去除卡片背景，改纯文本流（参考元宝对话风格：Prompt 标签 + 思考/调用/结果层级，无边框盒子感）
- 思考显示优化：去掉顶部"正在思考"动态占位气泡；思考默认折叠，仅生成中自动展开逐字书写；历史思考点击展开看完整推理
- 任务调度列表优化：无边框列表 + 状态色点（绿=完成/蓝=待执行），点行看详情、已完成不显示取消
- 本地模型（LiteRT）可直接下载运行：模型源 hf-mirror.com 国内镜像（4.68MB/s），5 个模型可选 + .litertlm 导入
- 本地模型开放任务调度工具（schedule/list/cancel_task）
### Fixes
- 修复思考区空白 bug（动画 key 改 animate，逐字书写正常）
### 备注
- 邮件/MCP 工具需云端模型；本地模型可用：任务调度、记忆、搜索、终端命令等
...
- AI 消息头部「AI 回复」信息栏 + 服务商/模型标签（fallback 时显示服务名），头部下加分隔线
- 工具卡片增强：左缘状态色条（绿=完成/暖=执行中/红=错误）、错误状态红标图标
- 用户消息「我」头部标签，与 AI 回复栏对称
- 消息列表垂直间距 8dp，思考/工具/结果分段层次更清晰
- 代码块行号列（编辑器风格，仿 CodeContentWithLineNumbers）
- 链接预览弹窗：点击消息内链接先弹预览确认再打开（更安全）
- 沙盒 Linux 打通访问安卓系统文件：/sdcard 与 /storage/emulated/0 挂载进 proot
- 工具国际化：安卓目录/图像生成中英文双语（跟随系统语言）
### Fixes
- 心跳不再报 OpenAICompatibleEmptyResponseException；心跳空响应不输出技术文案，强制中文汇报可见
### 备注
...
- 需求功能落地：REQ-2 思考强度选择器、REQ-3 流式输出开关、REQ-5.2 修改消息就地编辑、REQ-5.4 删除单条消息、REQ-9 MCP 工具人工确认开关
### Fixes
- 修复用户消息不显示 bug（UserMessage 花括号错位导致文本被嵌入附件分支）
- 修复心跳报错：响应 DTO `choices` 改为可空，避免缺失字段反序列化崩溃
- Free 档网络失败转友好空响应提示，不再报裸错误
## v3.2.3 — 2026-09-11 (Kai2 9000 正式版)
### Features
- 等待体验优化：思考内容逐步显示，去除轮播占位，请求期间插入实时思考气泡
- 错误体验优化：网络错误改友好文案（连接超时/重试引导），错误卡片升级为带文字的重试按钮
### Fixes
- 等待期不再报裸网络错误：Free 档网络失败转友好空响应提示
- 思考气泡与底部状态行去重，避免重复显示
## v3.2.2 — 2026-09-11 (Kai2 9000 正式版)
### Features
- 实时流水线显示：思考/工具调用/结果逐步实时展示，非执行完才显示
- 服务商自定义名称备注（设置 → 服务 → 编辑图标，显示"自定义名 (服务商原名)"）
- 主页 README 精简为仅安卓安装说明

### Fixes
- 心跳/定时任务无配置服务时回退 Free 免费档

## v3.2.1 — 2026-09-11 (Kai2 9000 正式版)
### Fixes
- 修复切换会话任务污染：任务跑私有 turnHistory 隔离，完成后按快照会话 ID 保存，不再把新会话内容拉进旧会话、不再覆盖删除旧记录
- 切换会话后暂停按钮自动同步（activeTurnConversationId）
- 历史消息任务运行绿灯标记

## v3.2.0 — 2026-09-11 (Kai2 9000 二开正式版)
### Features
- Kai2 9000 品牌二开：应用名、主页 README、赞助/合作链接改为 qtgf520
- 仅保留 Android 平台源码（删除 desktop/ios/wasm/flatpak/aur/fastlane/site）
- 新增 Android 目录操作工具（AI 可读写 /storage/emulated/0/ 下文件）
- 存储权限：READ/WRITE/MANAGE_EXTERNAL_STORAGE
- 工具调用流水线显示：运行中旋转沙漏/完成绿勾，可展开收起查看参数和结果
### Fixes
- 发送消息立即保存会话，切换聊天记录不再丢失刚发的消息
- 切换会话/新聊天不再取消正在执行的任务（任务绑定快照会话 ID）
- 聊天离开时任务不停止（进程级 appScope）
- 修复 ARM64 环境构建（aapt2 替换、手动打包、Coroutines Main dispatcher）
